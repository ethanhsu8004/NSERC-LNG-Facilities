---
title: "Flaring Maps LNG Facilities"
...
#MAPS
Flaring_Map_Freeport <- leaflet(Flaring_Data_Freeport_1KM) %>% addProviderTiles(providers$Esri.WorldImagery, options = tileOptions(maxZoom = 19))%>%  addCircleMarkers(lng = ~lon, lat = ~lat, clusterOptions = markerClusterOptions(maxClusterRadius = 25), color = "red", radius = 4) 


Flaring_Map_Corpus <- leaflet(Flaring_Data_Corpus_1KM) %>% addProviderTiles(providers$Esri.WorldImagery, options = tileOptions(maxZoom = 19)) %>%addCircleMarkers(lng = ~lon, lat = ~lat, clusterOptions = markerClusterOptions(maxClusterRadius = 25), color = "red", radius = 4) 

```

## FREEPORT FLARING MAP
```{r, echo = FALSE}
